...
echo Begin ReMuxing... 
for %%F in ("%INPUT%\*.%IN-EXT%") do (title ReMuxing %%~nF & set /a NumXX=NumXX+1 & set /p "=Job !NumXX! of %NumFiles% > %%~nF.%IN-EXT%..." <nul & %FFMPEG% %OVERWRITE% %LOGLEVEL% -i "%%F" -vcodec copy -c:a %A-CODEC% -b:a %A-BIT% -ac %A-CHAN% -map_metadata -1 -metadata:s:a:0 language=%A-LNG% -map_chapters -1 -movflags +faststart "%OUTPUT%\%%~nF.%OUT-EXT%" & (echo Success & set /a NumOK+=1) || (%CECHO% {red}Error!{#}{\n} & set /a NumError+=1 & %CECHO% {red}ffmpeg.exe reported an error on file %%~nF.%IN-EXT%{#}{\n}))
